...
public class DefaultProcessLoadStrategy implements ProcessLoadStrategy {
	 ...
	 */
	public ProcessRepository findTheProcessForRunning(WorkflowSession session,
			String workflowProcessId,String processType) throws InvalidModelException{
		...
		int v = processPersister.findTheLatestPublishedVersion(workflowProcessId, processType);
		if (v<=0) return null;
		ProcessRepository repository = processPersister.findProcessRepositoryByProcessKey(new ProcessKey(workflowProcessId,v,processType));
		return repository;
	}
	
	public ProcessKey findTheProcessKeyForRunning(WorkflowSession session,
			String workflowProcessId, String processType){
		PersistenceService persistenceService = rtCtx.getEngineModule(PersistenceService.class, processType);
		ProcessPersister processPersister = persistenceService.getProcessPersister();
