...
public class HandleFaultTest extends FireWorkflowJunitEnviroment {
	...
	public WorkflowProcess createWorkflowProcess(){
		...
		//异常处理子流程
		StartNodeImpl catchFaultStartNode = new StartNodeImpl(process,"catchFault");
		CatchFaultDecoratorImpl catchFaultDecorator = new CatchFaultDecoratorImpl();
		catchFaultDecorator.setErrorCode("ErrorCode001");
		catchFaultStartNode.setDecorator(catchFaultDecorator);
		
		ActivityImpl handleFaultAct = new ActivityImpl(process,"HandleFaultActivity");
		...
		process.getStartNodes().add(catchFaultStartNode);
	...
	public void assertResult(WorkflowSession session){
		...
		Assert.assertEquals(13, tokenList.size());
