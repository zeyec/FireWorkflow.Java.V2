 ...
 * @version 2.0
 */
public class CompensationTestWithCancellation extends FireWorkflowJunitEnviroment {
	protected static final String processName = "CompensationTestWithCancellation";
	...
	/**
	 * Start-->Router-->Activity1(with catch compenstion)-->End(throw compensation)
	 *       	  |         |-->HandleCompensation
	 *            |-->Activity2(human service)-->End2
	 *            
	 * 在这个流程中，throw compensation 的compensation code是  TheCompensationActivity，
	 * catch compensation的compensation code是缺省的，所以HandleCompensation不会被触发。  
	 * Activity2将被cancel掉。       
	 */
	...
	public void assertResult(WorkflowSession session){
		...
		Assert.assertEquals(ActivityInstanceState.CANCELLED, actInst2.getState());
		...
		Assert.assertEquals(WorkItemState.CANCELLED, wi.getState());
