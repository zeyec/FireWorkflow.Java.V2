 ...
 * @version 2.0
 */
public class TheAndSplitAndJoinRouter2 extends FireWorkflowJunitEnviroment {


	protected static final String processName = "TheAndSplitAndJoinRouter2Process";
	 ...
	 *             |---->Activity2-->Activity3-->Router2-->End
	 */
	public WorkflowProcess createWorkflowProcess(){
		...
		TransitionImpl transition5 = new TransitionImpl(process,"activity2_activity3");
		transition5.setFromNode(activity2);
		transition5.setToNode(activity3);
		activity2.getLeavingTransitions().add(transition5);
		activity3.getEnteringTransitions().add(transition5);
		
		TransitionImpl transition7 = new TransitionImpl(process,"activity3_router2");
		transition7.setFromNode(activity3);
		transition7.setToNode(router2);
		activity3.getLeavingTransitions().add(transition7);
		router2.getEnteringTransitions().add(transition7);		
		...
		process.getTransitions().add(transition7);
	...
	public void assertResult(WorkflowSession session){
		...
		Assert.assertEquals(16, tokenList.size());
		...
		//验证router2对应的ActivityInstance
		q4ActInst.reset();
		q4ActInst.add(Restrictions.eq(ActivityInstanceProperty.PROCESS_INSTANCE_ID, processInstanceId))
				.add(Restrictions.eq(ActivityInstanceProperty.NODE_ID, processName+".Router2"));
		List<ActivityInstance> actInstList4Router2 = q4ActInst.list();
		Assert.assertNotNull(actInstList4Router2);
		Assert.assertEquals(1, actInstList4Router2.size());
		activityInstance = actInstList4Router2.get(0);
		Assert.assertEquals(new Integer(9), activityInstance.getStepNumber());
	}	
